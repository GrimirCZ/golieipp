...
ARG TAGS="avahi"
RUN if [ -n "$TAGS" ]; then \
	CGO_ENABLED=0 GOOS=linux go build -tags "$TAGS" -trimpath -ldflags="-s -w" -o /out/golieipp ./cmd/golieipp; \
else \
	CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/golieipp ./cmd/golieipp; \
fi
